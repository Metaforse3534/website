...
export default function LegalPage({ path }: { path: string }) {
  ...
  return <Layout><Meta title={document.title} description={document.description} path={document.path} />
    <article className="legal-page"><header><p className="eyebrow">ORBIT LABS / LEGAL</p><h1>{document.title}</h1><p>{document.description}</p><div><span>Last updated: {document.updated}</span><button onClick={() => window.print()}><Printer size={15} />Print</button><button onClick={copy}><Clipboard size={15} />{copied ? 'Copied' : 'Copy link'}</button><a href="/Routes/contact">Contact <ExternalLink size={13} /></a></div></header><div className="legal-layout"><aside><p>CONTENTS</p>{document.sections.map((section, index) => <a key={section.title} href={`#legal-${index + 1}`}>{index + 1}. {section.title}</a>)}</aside><div className="legal-content">{document.notice && <p className="article-rule"><strong>Pre-launch notice:</strong> {document.notice}</p>}{document.sections.map((section, index) => <section id={`legal-${index + 1}`} key={section.title}><span>{String(index + 1).padStart(2, '0')}</span><h2>{section.title}</h2>{section.paragraphs.map(paragraph => <p key={paragraph}>{paragraph}</p>)}</section>)}</div></div>
    </article>
  </Layout>
}
